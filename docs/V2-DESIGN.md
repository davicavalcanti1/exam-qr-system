...
>
> **Nota de 17/ago/2026:** a **Frente C (NFS-e) saiu de escopo** e foi arquivada na branch
> `feat-nfse` — ver §6. Em seu lugar entra a **assinatura eletrônica via ZapSign**
> (contrato de parceria + DPA), documentada em [ZAPSIGN.md](./ZAPSIGN.md).
> As Frentes A (pagamento real) e B (preço via `consultarPreco`) seguem de pé.
...
> **Status em 17/ago/2026 — frente arquivada, fora da master.**
> A Frente C chegou a ser implementada por inteiro (client da Focus, rotas `/api/nfse`,
> tela `NfseConfig`, migration `20260727120000` — nunca aplicada) e vive hoje na branch
> **`feat-nfse`** (commit `045dd1a`). Foi tirada da master porque o lugar dela na aba de
> integrações passou a ser o **ZapSign** (assinatura eletrônica de contrato e DPA).
> O desenho abaixo segue válido caso a emissão volte à pauta.
