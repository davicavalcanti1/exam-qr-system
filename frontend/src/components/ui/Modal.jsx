...
export function Modal({ open = true, onClose, title, subtitle, icon, children, footer, size = 'md', className = '' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fadein" onClick={onClose}>
      <div className={`bg-surface-container-lowest rounded-2xl shadow-2xl w-full ${SIZES[size]} max-h-[90vh] flex flex-col overflow-hidden animate-popin ${className}`} onClick={e => e.stopPropagation()}>
        {(title || onClose) && (
          <div className="flex items-center justify-between gap-3 p-5 border-b border-outline-variant/10 flex-none">
